...
class ToolCallingBoilerplateAgent(AgentProtocol):
    ...
    def _get_approval_required_tools(self) -> set[str]:
    ...
    def process_tool_calls(
        ...
        approval_required_tools: list[ToolCall] = []
            ...
            if content_block.get("type") == "tool_use":
                ...
                if tool_name in self.approval_required_tools:
                    ...
                    tool_call = self.create_tool_call_for_approval(
                        tool_name or "", tool_input, tool_use_id or ""
                    )
...
Be surgical, simple and less wordy."""
        ...
        result: dict[str, Any] = self.llm.invoke(
    ...
    def invoke(self, messages: dict[str, list[dict[str, Any]]]) -> AgentMessage:
        ...
        if executed_tool_calls:
            for executed_tool_call in executed_tool_calls:
                ...
                conversation_messages.append(
                    {"role": "user", "content": tool_execution_result_content}
                )
        ...
        while True:
            ...
            if final_response_call:
                ...
                agent_message = AgentMessage(content=final_input.get("content", ""))

                # Add terminal commands to data if present
                if final_input.get("terminal_commands"):
                    for cmd in final_input.get("terminal_commands", []):
                        agent_message.data.cmds.append(
                            Command(command=cmd.get("command"), execute=False)
                        )
            ...
            if other_tool_calls:
                ...
                conversation_messages.append(
                    {"role": "assistant", "content": "Processing Tool Calls"}
                )

                # Process tools - execute non-approval ones, collect approval-required ones
                executed_tool_calls, approval_required_tools = self.process_tool_calls(
                    response_content
                )
