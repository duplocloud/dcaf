...
from ..schemas.messages import AgentMessage, ExecutedToolCall, ToolCall
...
class ToolCallingAgent:
    ...
    def __init__(
        ...
        llm_visible_platform_context_fields: list[str] | None = None,
    ):
        """
        ...
        """
        if llm_visible_platform_context_fields is None:
            llm_visible_platform_context_fields = ["tenant_name"]
        self.llm = llm
    ...
    def _build_tool_schemas(self) -> list[dict[str, Any]]:
        ...
        if self.enable_terminal_cmds:
            input_schema = final_response_schema["input_schema"]
            if isinstance(input_schema, dict):
                props = input_schema.get("properties", {})
                if isinstance(props, dict):
                    props["terminal_commands"] = {
                        "type": "array",
    ...
    def execute_tool(
        self, tool_name: str, tool_input: dict[str, Any], platform_context: dict[str, Any]
    ) -> str:
    ...
    def process_approved_tool_calls(
        self, messages: dict[str, list[dict[str, Any]]], platform_context: dict[str, Any]
            ...
            for tool_call in tool_calls:
                if tool_call.get("execute", False):
                    ...
                    result = self.execute_tool(
                        tool_call["name"], tool_call["input"], platform_context
                    )
    ...
    def process_tool_calls(
        self, response_content: list[dict[str, Any]], platform_context: dict[str, Any]
        ...
        for tool_use_block in response_content:
            ...
            if tool_name in self.tools:
                ...
                if tool.requires_approval:
                    ...
                    # Execute immediately
                    result = self.execute_tool(tool_name, tool_input, platform_context)
                    executed_tool_calls.append(
                        ExecutedToolCall(
                            id=tool_use_id or "", name=tool_name, input=tool_input, output=result
                        )
                    )
    ...
    def preprocess_messages(
            ...
            if role == "user":
                ...
                if current_context and (is_first_user_message or context_has_changed):
                    ...
                    processed_msg["content"] = (
                        context_header + "User Message: " + processed_msg["content"]
                    )
        ...
        if user_message_positions and last_context:
            ...
            final_context_header = (
                "Current Message Context:\n- " + "\n- ".join(context_lines) + "\n\n"
            )

            # Always use original content as the base - no parsing needed
            preprocessed[final_user_position]["content"] = (
                final_context_header + "User Message: " + original_final_content
            )
    ...
    def invoke(
        ...
        executed_tool_calls = self.process_approved_tool_calls(messages, platform_context)
        ...
        for iteration in range(self.max_iterations):
            ...
            if other_tool_calls:
                conversation.append({"role": "user", "content": "Processing tool calls..."})
            ...
            if final_response_call:
                ...
                agent_message = AgentMessage(content=final_response)
