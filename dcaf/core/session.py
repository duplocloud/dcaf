...
def _is_pydantic_model_class(cls: Any) -> bool:
    ...
    return hasattr(cls, "model_validate") and callable(cls.model_validate)
...
def _serialize_value(value: Any) -> Any:
    ...
    if is_dataclass(value) and not isinstance(value, type) and not _is_pydantic_model(value):
        return asdict(value)
...
def _deserialize_value(data: Any, as_type: type[T] | None) -> T | Any:
    ...
    if _is_pydantic_model_class(as_type) and hasattr(as_type, "model_validate"):
        return as_type.model_validate(data)  # type: ignore[attr-defined]
