...
RequestInterceptor = Callable[
    [LLMRequest], 
    Union[LLMRequest, Awaitable[LLMRequest]]
]
...
class InterceptorPipeline:
    ...
    def __init__(
        ...
        logger.debug(
            f"Created {pipeline_name} interceptor pipeline with "
            f"{len(interceptors)} interceptor(s)"
        )
    ...
    async def run(self, data: Any) -> Any:
        ...
        for interceptor_index, interceptor_function in enumerate(self.interceptors):
            ...
            interceptor_name = getattr(
                ...
                "__name__", 
                f"interceptor_{interceptor_index}"
            )
            
            logger.debug(
                f"Running {self.pipeline_name} interceptor: {interceptor_name}"
            )
        ...
        logger.debug(
            f"Completed {self.pipeline_name} interceptor pipeline successfully"
        )
        
        return current_data
    
    def run_sync(self, data: Any) -> Any:
        """
        Run all interceptors synchronously.
        
        This is a convenience method for when you're not in an async context.
        It creates an event loop if needed and runs the async version.
        ...
        Note:
            If you're already in an async context, use run() instead.
        """
        try:
            ...
            if loop.is_running():
                ...
                raise RuntimeError(
                    "Cannot use run_sync() inside an async context. "
                    "Use 'await pipeline.run()' instead."
                )
            return loop.run_until_complete(self.run(data))
        except RuntimeError:
