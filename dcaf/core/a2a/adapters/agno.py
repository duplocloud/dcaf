...
class AgnoA2AServer(A2AServerAdapter):
    ...
    def create_routes(self, agent: "Agent") -> list["APIRouter"]:
        ...
        if not hasattr(agent, "_a2a_task_manager"):
            agent._a2a_task_manager = A2ATaskManager()
        ...
        async def send_task(task_data: dict, request: Request, async_mode: bool = False):
            """
            ...
            try:
                ...
                if async_mode or request.query_params.get("async") == "true":
                    ...
                    task_id = agent._a2a_task_manager.create_task(task)
                    
                    # TODO: Execute task in background
                    # For now, just execute synchronously
                    result = self.handle_task(agent, task)
                    agent._a2a_task_manager.update_task(task_id, result)
        ...
        async def get_task_status(task_id: str):
            """Get the status of an async task."""
            try:
                result = agent._a2a_task_manager.get_task(task_id)
                return result.to_dict()
            except KeyError:
    ...
    def handle_task(self, agent: "Agent", task: Task) -> TaskResult:
        """
        ...
        try:
            ...
            # Execute the agent
            response = agent.run(messages=messages, context=task.context)
