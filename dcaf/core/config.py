...
def get_env(key: str, default: Any = None, cast: type = str) -> Any:
    ...
    if cast == bool:
        ...
        try:
            ...
            logger.warning(f"Invalid {cast.__name__} value for {key}: {value}, using default: {default}")
            return default
...
def load_agent_config(
    ...
    **overrides: Any
    ...
    config["model"] = (
        model or
        get_env(EnvVars.MODEL) or
        PROVIDER_MODEL_DEFAULTS.get(config["provider"]) or
        DEFAULTS["model"]
    )
...
def _log_config(config: Dict[str, Any]) -> None:
    ...
    safe_config = {
        ...
        if k not in ("api_key", "aws_access_key", "aws_secret_key")
    }
...
def get_provider_from_env() -> str:
    ...
    """
    return get_env(EnvVars.PROVIDER, DEFAULTS["provider"])


def get_model_from_env(provider: Optional[str] = None) -> str:
    """
    ...
    return (
        get_env(EnvVars.MODEL) or
        PROVIDER_MODEL_DEFAULTS.get(provider) or
        DEFAULTS["model"]
    )
...
def is_provider_configured(provider: str) -> bool:
    ...
    if provider == "bedrock":
        ...
        return bool(
            get_env(EnvVars.AWS_PROFILE) or
            (get_env(EnvVars.AWS_ACCESS_KEY_ID) and get_env(EnvVars.AWS_SECRET_ACCESS_KEY))
        )
