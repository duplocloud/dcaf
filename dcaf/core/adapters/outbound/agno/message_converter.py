...
from ....domain.value_objects import MessageContent, ContentBlock, ContentType
from ....application.dto.responses import AgentResponse, ToolCallDTO, StreamEvent, StreamEventType
...
class AgnoMessageConverter:
    ...
    def from_agno(
        ...
        return AgentResponse(
            ...
            is_complete=len(tool_calls) == 0 or all(
                tc.status != "pending" for tc in tool_calls
            ),
        )
    ...
    def _convert_message_to_agno(self, message: Message) -> Optional[AgnoMessage]:
    ...
    def _convert_content_from_agno(
        ...
        for agno_block in content:
            ...
            if block_type == "text":
                blocks.append(ContentBlock.text_block(agno_block.get("text", "")))
            elif block_type == "tool_use":
                blocks.append(ContentBlock.tool_use_block(
                    tool_use_id=agno_block.get("id", ""),
                    tool_name=agno_block.get("name", ""),
                    tool_input=agno_block.get("input", {}),
                ))
            elif block_type == "tool_result":
                blocks.append(ContentBlock.tool_result_block(
                    tool_use_id=agno_block.get("tool_use_id", ""),
                    result=agno_block.get("content", ""),
                ))
