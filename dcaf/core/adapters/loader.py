...
def load_adapter(framework: str, **kwargs: Any) -> RuntimeAdapter:
    """
    ...
    try:
        ...
        if available:
            ...
            raise ValueError(
                f"Unknown framework: '{framework}'. Available frameworks: {available_str}"
            ) from e
        else:
            raise ValueError(
                f"Unknown framework: '{framework}'. No frameworks found in {ADAPTERS_PACKAGE}"
            ) from e
