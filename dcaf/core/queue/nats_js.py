...
class NatsJobQueue(JobQueue):
    ...
    async def connect(self) -> None:
        """Connect to NATS and ensure the DCAF_JOBS_IN stream exists."""
    ...
    async def _ensure_in_stream(self) -> None:
        ...
        try:
            await self._js.stream_info(JOBS_IN_STREAM)  # type: ignore[union-attr]
    ...
    async def _ensure_consumer(
        ...
        try:
            await self._js.consumer_info(JOBS_IN_STREAM, durable)  # type: ignore[union-attr]
        except nats.js.errors.NotFoundError:
            await self._js.add_consumer(JOBS_IN_STREAM, cfg)  # type: ignore[union-attr]
            logger.info(
    ...
    async def publish(self, subject: str, message: BaseModel) -> None:
        ...
        await self._js.publish(subject, message.model_dump_json().encode())  # type: ignore[union-attr]
    ...
    async def subscribe(
        ...
        sub = await self._js.pull_subscribe(subject, durable=durable, stream=JOBS_IN_STREAM)  # type: ignore[union-attr]
