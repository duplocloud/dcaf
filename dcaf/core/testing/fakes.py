...
from ..application.ports.approval_callback import ApprovalDecision
...
class FakeConversationRepository:
    ...
    @property
    def last_saved(self) -> Conversation | None:
...
class FakeApprovalCallback:
    ...
    def request_approval(
        ...
        if self._auto_approve:
            return [ApprovalDecision.approve(str(tc.id)) for tc in tool_calls]

        if self._auto_reject:
            return [
                ApprovalDecision.reject(str(tc.id), self._auto_reject_reason) for tc in tool_calls
            ]
