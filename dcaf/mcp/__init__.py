...
# Import MCPTool and MCPToolCall for connecting to external MCP servers
from .tools import MCPTool, MCPToolCall
...
def serve(
    ...
    additional_resources: list[tuple] | None = None,
...
def _register_tool(mcp, tool) -> None:
    ...
    if tool.requires_platform_context:
        ...
        try:
            ...
            filtered_params = [
                p for name, p in sig.parameters.items() if name != "platform_context"
            ]
            wrapper.__signature__ = sig.replace(parameters=filtered_params)  # type: ignore[attr-defined]
...
def _add_agent_chat_tool(mcp, agent: "Agent", server_name: str) -> None:
    ...
    mcp.tool(name="chat", description=f"Chat with {server_name} (full agent with LLM reasoning)")(
        chat_with_agent
    )
...
def _add_chat_endpoints(mcp, agent: "Agent") -> None:
    ...
    async def health_check(request: Request):  # noqa: ARG001 - Required by Starlette route signature
    ...
    async def chat_endpoint(request: Request):
        ...
        try:
            ...
            return JSONResponse(
                {"role": "assistant", "content": f"Error: {str(e)}", "data": {}}, status_code=500
            )
    ...
    async def chat_stream_endpoint(request: Request):
        ...
        try:
            ...
            async def generate():
                try:
                    result = await agent.run(messages=messages, context=context)
...
def _to_helpdesk_response(result: Any) -> dict[str, Any]:
    ...
    for executed in result.executed_tools:
        data["executed_tool_calls"].append(
            {
                "id": executed.id,
                "name": executed.name,
...
def serve_mcp(
    agent: "Agent",
    name: str | None = None,
    version: str = "1.0.0",  # noqa: ARG001 - Deprecated, kept for API compatibility
    transport: str = "sse",  # noqa: ARG001 - Deprecated, kept for API compatibility
    host: str = "0.0.0.0",
    port: int = 8000,
) -> None:
    """
...
def create_mcp_server(
    ...
    name: str = "dcaf-agent",
    version: str = "1.0.0",
    include_agent_tool: bool = True,  # noqa: ARG001 - Deprecated, kept for API compatibility
):
    """
