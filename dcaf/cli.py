...
def env_update_aws_creds(tenant=None, host=None):
    ...
    if not shutil.which("duplo-jit"):
        ...
        print(
            "Please install duplo-jit: https://docs.duplocloud.com/docs/overview/use-cases/jit-access#step-1.-install-duplo-jit"
        )
        return 1
...
def docker_build_push_ecr(
    tag, repo_name=None, registry=None, aws_profile=None, region=None, dockerfile="Dockerfile"
):
    ...
    if not repo_name:
        print(
            "Error: Repository name not specified. Set ECR_REPOSITORY_NAME env var or use --repo-name flag"
        )
        return 1
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="dcaf", description="DuploCloud Agent Builder CLI")

    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # env-update-aws-creds command
    env_parser = subparsers.add_parser(
        "env-update-aws-creds", help="Update AWS credentials in .env file from DuploCloud"
    )
    ...
    docker_parser = subparsers.add_parser(
        "docker-build-push-ecr", help="Build and push Docker image to Amazon ECR"
    )
    docker_parser.add_argument("tag", help="Image tag (e.g., latest, v1.0.0)")
    docker_parser.add_argument(
        "--repo-name", help="ECR repository name (or set ECR_REPOSITORY_NAME)"
    )
    ...
    docker_parser.add_argument(
        "--dockerfile", default="Dockerfile", help="Path to Dockerfile (default: Dockerfile)"
    )

    # deploy-agent command
    deploy_parser = subparsers.add_parser("deploy-agent", help="Deploy agent to DuploCloud")
    ...
    if args.command == "env-update-aws-creds":
        ...
        exit_code = docker_build_push_ecr(
            args.tag, args.repo_name, args.registry, args.aws_profile, args.region, args.dockerfile
        )
