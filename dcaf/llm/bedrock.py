...
class BedrockLLM(LLM):
    ...
    def invoke_stream(
        ...
        if additional_params:
            ...
            if isinstance(inference_config, dict):
                inference_config.update(additional_params)
    ...
    def invoke(
        ...
        **_kwargs: Any,
        ...
        if tools:
            formatted_tool_config = self._format_tool_config(tools, tool_choice)
            request["toolConfig"] = formatted_tool_config  # type: ignore[assignment]

        # Add additional model-specific fields if provided
        if additional_model_request_fields:
            request["additionalModelRequestFields"] = additional_model_request_fields  # type: ignore[assignment]
        ...
        if performance_config and "latency" in performance_config:
            request["performanceConfig"] = {"latency": performance_config["latency"]}  # type: ignore[assignment]
    ...
    def normalize_message_roles(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if len(merged) < len(messages):
            ...
            if (
                m := next((m for m in reversed(merged) if m.get("role") == "assistant"), None)
            ) and isinstance(m.get("content", ""), str):
                m["content"] = m["content"].rstrip()
            return merged

    def _merge_message_content(
        self, target_msg: dict[str, Any], source_msg: dict[str, Any]
    ) -> None:
        """
    ...
    def _format_tool_config(
        self, tools: list[dict[str, Any]], tool_choice: str | dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
