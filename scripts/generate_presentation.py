...
def create_presentation():
    ...
    # Slide 1: Title
    add_title_slide(
        prs, "DCAF Core", "Building AI Agents with Simplicity", "DuploCloud Agent Framework"
    )
...
def add_code_slide(prs, title, code, footer=None):
    ...
    code_bg = slide.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.4), Inches(1.3), Inches(12.5), Inches(5.0)
    )
    code_bg.fill.solid()
...
def add_table_slide(prs, title, headers, rows):
    ...
    table = slide.shapes.add_table(
        num_rows, num_cols, Inches(0.5), Inches(1.5), Inches(12.333), Inches(0.5 * num_rows)
    ).table
...
def add_architecture_slide(prs):
    ...
    add_arch_box(
        ...
        1.5,
        "DCAF Core",
        "HTTP → Messages → Agent Logic → Approvals → Response",
        GREEN,
    )
