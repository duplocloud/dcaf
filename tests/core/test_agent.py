...
class TestAgentInit:
    ...
    def test_event_handlers_single_callable(self):
        handler = lambda e: None  # noqa: E731
        agent = _create_agent(on_event=handler)
        assert agent._event_handlers == [handler]

    def test_event_handlers_list(self):
        h1 = lambda e: None  # noqa: E731
        h2 = lambda e: None  # noqa: E731
        agent = _create_agent(on_event=[h1, h2])
        assert agent._event_handlers == [h1, h2]
...
class TestBuildSystemParts:
    ...
    def test_callable_system_context(self):
        ...
        static, dynamic = agent._build_system_parts(
            platform_context={"tenant_name": "prod"}
        )
        assert static is None
...
# =============================================================================
# Agent._create_event_publisher Tests
# =============================================================================
...
class TestCreateEventPublisher:
    def test_no_handlers_returns_none(self):
        agent = _create_agent()
        publisher = agent._create_event_publisher()
        assert publisher is None

    def test_with_handlers_returns_publisher(self):
        events_received = []
        agent = _create_agent(on_event=lambda e: events_received.append(e))
        publisher = agent._create_event_publisher()
        assert publisher is not None
        assert hasattr(publisher, "publish")
        assert hasattr(publisher, "publish_all")

    def test_publisher_calls_handlers(self):
        events_received = []
        agent = _create_agent(on_event=lambda e: events_received.append(e))
        publisher = agent._create_event_publisher()

        # Simulate publishing an event (just pass any object)
        publisher.publish("test-event")
        assert events_received == ["test-event"]

    def test_publisher_publish_all(self):
        events_received = []
        agent = _create_agent(on_event=lambda e: events_received.append(e))
        publisher = agent._create_event_publisher()

        publisher.publish_all(["event1", "event2"])
        assert events_received == ["event1", "event2"]

    def test_publisher_handler_error_does_not_propagate(self):
        def bad_handler(event):
            raise RuntimeError("handler error")

        agent = _create_agent(on_event=bad_handler)
        publisher = agent._create_event_publisher()
