...
class TestContentDoesNotContainBrackets:
    ...
    async def test_tool_call_response_no_brackets(self, agno_adapter, tools):
        ...
        """
        messages = [
            {"role": "user", "content": "List the pods running in the default namespace"}
        ]
        
        response = await agno_adapter.ainvoke(
            messages=messages,
            tools=tools,
...
class TestDCAFAgentNoBrackets:
    ...
    async def test_agent_run_no_brackets(self, dcaf_agent):
        ...
        messages = [
            {"role": "user", "content": "Check what processes are running on the system"}
        ]
...
class TestContentTypeInspection:
    ...
    async def test_inspect_run_output_content_type(self, agno_adapter, tools):
        ...
        if run_output.content is not None:
            logger.info(f"run_output.content class: {run_output.content.__class__.__module__}.{run_output.content.__class__.__name__}")
        ...
        if '[]' in content_str:
            logger.warning(f"FOUND [] in content!")
        ...
        if run_output.content:
            ...
            assert not content_repr.endswith("[]'"), (
                f"Content ends with []!\n"
                f"Type: {type(run_output.content)}\n"
                f"Repr: {content_repr}"
            )
...
class TestWithAndWithoutTools:
    ...
    async def test_same_prompt_with_and_without_tools(self, agno_adapter, tools):
        ...
        # With tools
        response_with_tools = await agno_adapter.ainvoke(
            messages=messages,
            tools=tools,
        ...
        # Without tools
        response_without_tools = await agno_adapter.ainvoke(
            messages=messages,
            tools=[],
            system_prompt="You are a helpful assistant.",
        )
        ...
        # Check both
        if response_with_tools.text:
            has_brackets_with = '[]' in response_with_tools.text
        else:
            has_brackets_with = False
